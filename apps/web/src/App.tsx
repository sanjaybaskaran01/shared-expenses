...
function GroupRail(props: { activeGroupId?: string | undefined; onSelect(groupId: string): void; onCreate(): void }) {
  return <div class="group-rail -mx-4 flex snap-x gap-3 overflow-x-auto px-4 pb-1 sm:mx-0 sm:px-0">
    <For each={appStore.groups()}>{(group) => {
      ...
      return <button type="button" class="group-tile relative min-w-[14rem] snap-start overflow-hidden rounded-2xl border p-4 text-left transition-transform active:scale-[.98]" classList={{ selected: props.activeGroupId === group.id }} onClick={() => props.onSelect(group.id)}><div><div class="flex items-center justify-between"><Avatar name={group.name} class="group-avatar" /><Show when={props.activeGroupId === group.id}><span class="group-selected-mark"><CheckCircle2 size={15} /></span></Show></div><strong class="mt-5 block truncate text-lg tracking-[-.02em]">{group.name}</strong><span class="group-tile-detail mt-1 block text-xs">{expenses().length} {expenses().length === 1 ? "expense" : "expenses"} · {money(total(), group.settlementCurrency, true)}</span></div></button>;
    }}</For>
...
function GroupsView(props: { actorId: string; activeGroupId?: string | undefined; onSelectGroup(groupId: string): void; onAddExpense(groupId?: string): void; onOpenExpense(expense: LocalExpense): void; onCreateGroup(): void; onSettle(settlement: Settlement | undefined, currency: string): void }) {
  ...
  return <div class="page-enter space-y-5 sm:space-y-6">
    ...
    <Show when={group()} keyed fallback={<Card class="grid min-h-72 place-items-center p-8 text-center"><div><UsersRound class="mx-auto text-muted-foreground" size={30} /><h2 class="mt-3 font-semibold">Create your first group</h2><p class="mt-1 text-sm text-muted-foreground">Add people, then start logging shared expenses.</p><Button class="mt-5" onClick={props.onCreateGroup}><Plus size={16} /> New group</Button></div></Card>}>{(activeGroup) => <>
      <section class="balance-hero overflow-hidden rounded-[1.75rem] p-5 text-white sm:p-6"><div class="flex items-start justify-between gap-4"><div><p class="text-xs font-medium text-white/60">{activeGroup.name} · your balance</p><strong class="amount-display mt-2 block text-4xl font-semibold tabular-nums">{yourBalance() === 0 ? money(0, currency()) : `${yourBalance() > 0 ? "+" : "−"}${money(Math.abs(yourBalance()), currency())}`}</strong><p class="mt-1 text-sm text-white/70">{yourBalance() === 0 ? "Everyone is settled up" : yourBalance() > 0 ? "You get back" : "You owe"}</p></div><select class="glass-select" value={currency()} onInput={(event) => setCurrency(event.currentTarget.value)} aria-label="Balance currency"><For each={currenciesFor(activeGroup.id)}>{(item) => <option value={item}>{item}</option>}</For></select></div><div class="mt-7 grid grid-cols-3 gap-2"><button class="hero-action" onClick={() => props.onAddExpense(activeGroup.id)}><Plus size={17} /><span>Add</span></button><button class="hero-action" onClick={() => props.onSettle(settlements()[0], currency())}><WalletCards size={17} /><span>Settle</span></button><button class="hero-action" onClick={() => document.getElementById("insights")?.scrollIntoView({ behavior: "smooth" })}><BarChart3 size={17} /><span>Totals</span></button></div></section>
...
function AuthenticatedApp(props: { actorId: string }) {
  ...
  const [preferredTargetGroupId, setPreferredTargetGroupId] = createSignal<string>();
  ...
  function addExpense(id?: string) { setPreferredTargetGroupId(id ?? selectedGroupId()); setEditingExpense(undefined); setTargetPickerOpen(true); }
  function chooseExpenseTarget(target: ExpenseTarget) { setExpenseTarget(target); setSelectedGroupId(target.groupId); setTargetPickerOpen(false); setExpenseOpen(true); }
  ...
  return <div class="app-shell min-h-dvh text-foreground md:grid md:grid-cols-[16rem_minmax(0,1fr)]">
    ...
    <button class="floating-add md:hidden" onClick={() => addExpense()} aria-label="Add expense"><Plus size={24} /></button>
    ...
    <ExpenseTargetPicker open={targetPickerOpen()} actorId={props.actorId} preferredGroupId={preferredTargetGroupId()} onOpenChange={setTargetPickerOpen} onSelect={chooseExpenseTarget} onCreateGroup={() => setGroupOpen(true)} />
    <ExpenseComposer open={expenseOpen()} actorId={props.actorId} initialGroupId={editingExpense()?.groupId ?? expenseTarget()?.groupId ?? selectedGroupId()} initialParticipantIds={editingExpense() ? undefined : expenseTarget()?.participantIds} targetLabel={editingExpense() ? undefined : expenseTarget()?.label} expense={editingExpense()} onOpenChange={(open) => { setExpenseOpen(open); if (!open) setEditingExpense(undefined); }} onChangeTarget={() => { setExpenseOpen(false); setTargetPickerOpen(true); }} onSaved={(mode) => notify(mode === "updated" ? "Expense updated" : "Expense added")} />
