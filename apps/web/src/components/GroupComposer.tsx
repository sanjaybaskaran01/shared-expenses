    ...
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <Dialog.Portal>
        ...
        <div class="fixed inset-0 z-50 grid items-end sm:place-items-center sm:p-6">
          <Dialog.Content
            role="dialog"
            class="composer-dialog w-full rounded-t-2xl border border-border bg-card shadow-2xl outline-none sm:max-w-md sm:rounded-xl"
          ...
          >
            <header class="flex min-h-14 items-center justify-between border-b border-border px-5">
              <div><Dialog.Title class="text-base font-semibold">Create a group</Dialog.Title><Dialog.Description class="text-xs text-muted-foreground">Keep a trip, home, or event separate.</Dialog.Description></div>
