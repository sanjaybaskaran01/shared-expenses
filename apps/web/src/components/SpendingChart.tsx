...
export function SpendingChart(props: SpendingChartProps) {
  ...
  function render(): void {
    ...
    const palette = [
      styles.getPropertyValue("--chart-blue").trim(),
      styles.getPropertyValue("--chart-gold").trim(),
      styles.getPropertyValue("--chart-rose").trim(),
      styles.getPropertyValue("--chart-lilac").trim(),
      accent,
    ].filter(Boolean);
    ...
    if (props.mode === "category") {
      ...
      chart.setOption({
        ...
        series: [{ type: "bar", data: entries.map(([, value], index) => ({ value, itemStyle: { color: palette[index % palette.length] ?? accent } })), barWidth: 10, itemStyle: { borderRadius: 8 }, label: { show: true, position: "right", color: muted, fontSize: 10, formatter: (item: { value: number }) => money(item.value, props.currency) } }],
      }, true);
      return;
    }
    const totals = new Map<string, number>();
