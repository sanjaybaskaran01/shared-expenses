...
const members: ExpenseLanguageMember[] = [
  { userId: "me", displayName: "Sanjay Baskaran", isActor: true },
  { userId: "ananya", displayName: "Ananya Chembai" },
...
describe("natural-language expense parsing", () => {
  test("requires a payer instead of assuming the actor paid", () => {
    const result = parse("Lunch with Ananya, Rishi for $35 and it was split");

    expect(result.description).toBe("Lunch");
    expect(result.amount).toBe("35.00");
    expect(result.currency).toBe("USD");
    expect(result.participantIds).toEqual(["me", "ananya", "rishi"]);
  ...
  test("completes unspecified percentages by sharing the remainder", () => {
    const result = parse("Lunch with Ananya and Rishi but Ananya ate 30% of it, bill was for $35");

    expect(result.participantIds).toEqual(["me", "ananya", "rishi"]);
    expect(result.splitMethod).toBe("percentage");
    expect(result.splitValues).toEqual({ me: "35", ananya: "30", rishi: "35" });
    expect(result.status).toBe("needs-review");
    expect(result.issues).toContainEqual(expect.objectContaining({ code: "payer-unspecified" }));
  });
  ...
  test("handles a single-word display name in assigned splits", () => {
    const result = parseExpenseLanguage("Dinner with Alex but Alex had 30%, bill was $35", {
      members: [
        ...
        { userId: "me", displayName: "Sanjay", isActor: true },
      ],
      defaultCurrency: "USD",
      now: today,
    });
  ...
  test("resolves a non-actor payer", () => {
    const result = parse("Ananya paid $48 for dinner with me and Rishi, split equally");

    expect(result.description).toBe("Dinner");
    expect(result.payerIds).toEqual(["ananya"]);
    expect(result.participantIds).toEqual(["me", "ananya", "rishi"]);
    expect(result.splitMethod).toBe("equal");
  });

  test("understands multiple payer amounts", () => {
    const result = parse("Cab for $35 with Ananya and Rishi; I paid $20 and Ananya paid $15");

    expect(result.payerIds).toEqual(["me", "ananya"]);
    expect(result.payerValues).toEqual({ me: "20.00", ananya: "15.00" });
    expect(result.status).toBe("ready");
  });

  test("fills an unspecified exact share from the remaining total", () => {
    const result = parse("Groceries with Ananya and Rishi for $60; Ananya owes $10, Rishi owes $20, rest is mine");

    expect(result.splitMethod).toBe("exact");
    expect(result.splitValues).toEqual({ me: "30.00", ananya: "10.00", rishi: "20.00" });
    expect(result.status).toBe("needs-review");
    expect(result.issues).toContainEqual(expect.objectContaining({ code: "payer-unspecified" }));
  });

  test("understands weighted shares", () => {
    const result = parse("Pizza $40 with Ananya and Rishi, Ananya gets 2 shares and the rest 1 share each");

    expect(result.splitMethod).toBe("shares");
    expect(result.splitValues).toEqual({ me: "1", ananya: "2", rishi: "1" });
  });

  test("understands adjustments", () => {
    const result = parse("Utilities $90 with Ananya and Rishi; Ananya pays $6 more and Rishi $6 less");

    expect(result.splitMethod).toBe("adjustment");
    expect(result.splitValues).toEqual({ me: "0.00", ananya: "6.00", rishi: "-6.00" });
  });

  test("can split between other people without including the payer", () => {
    const result = parse("I paid $30 for snacks, split between Ananya and Rishi");

    expect(result.payerIds).toEqual(["me"]);
    expect(result.participantIds).toEqual(["ananya", "rishi"]);
    expect(result.splitMethod).toBe("equal");
  });
  ...
  test("uses current participants when names are omitted", () => {
    const result = parseExpenseLanguage("Coffee $12", {
      members,
      defaultCurrency: "USD",
      defaultParticipantIds: ["me", "ananya"],
      now: today,
    });

    expect(result.participantIds).toEqual(["me", "ananya"]);
    expect(result.chips).toContainEqual(expect.objectContaining({ field: "participants", value: "you, Ananya" }));
  });

  test("understands dates, recurrence, and non-dollar currencies", () => {
    const result = parse("Train tickets with Ananya for ₹1,250 yesterday, repeat monthly");
  ...
  test("treats an ambiguous dollar sign as the selected group's currency", () => {
    const result = parseExpenseLanguage("Coffee with Ananya for $12", {
      members,
      defaultCurrency: "CAD",
      now: today,
    });
  ...
  test("reports unknown people instead of inventing members", () => {
    const result = parse("Lunch with Ananya and Priya for $35");
  ...
  test("reports required information without guessing", () => {
    const result = parse("Lunch with Ananya");
  ...
  test("routes hedged splits to review instead of accepting an estimate", () => {
    const result = parse("I paid $35 for lunch with Ananya and Rishi; Ananya had maybe 30%");
  ...
  test("routes refunds and transfers to review instead of auto-adding an expense", () => {
    const result = parse("I received a refund of $18 for the dinner with Ananya");
  ...
  test("does not confuse a place or service name with an uncertain amount or a transfer", () => {
    const result = parse("I paid $20 for a taxi around town and $12 for an airport transfer with Ananya");
  ...
  test("makes an unstated date visible as a default", () => {
    const result = parse("I paid $12 for coffee with Ananya");
  ...
  test("stays well below the interaction latency budget", () => {
    const scenarios = [
      "Lunch with Ananya, Rishi for $35 and it was split",
      "Lunch with Ananya and Rishi but Ananya ate 30% of it, bill was for $35",
      "Cab for $35 with Ananya and Rishi; I paid $20 and Ananya paid $15",
      "Groceries with Ananya and Rishi for $60; Ananya owes $10, Rishi owes $20, rest is mine",
    ];
