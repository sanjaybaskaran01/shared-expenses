...
const serverSnapshot: ScenarioServerSnapshot = {
  ...
  expenses: [{
    ...
    version: 1,
    amountMinor: 12_000,
    payers: [{ participantId: "maya", amountMinor: 12_000 }],
    allocations: [
      { participantId: "maya", amountMinor: 3_000 },
...
describe("scenario lab model", () => {
  test("defines four stable, unique people", () => {
    ...
    expect(DEFAULT_SCENARIO_ACTORS.map(({ name }) => name)).toEqual(["Maya", "Dev", "Mira", "Arjun"]);
  });
  ...
  test("proves allocation integrity and zero-sum balances", () => {
    ...
    expect(result.balances).toEqual({ maya: 8_000, arjun: -3_000, dev: -2_000, mira: -3_000 });
  });
