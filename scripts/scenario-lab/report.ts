...
function reportHtml(report: ScenarioRunReport): string {
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">${report.status === "running" ? '<meta http-equiv="refresh" content="2">' : ""}<title>Tallied Scenario Lab · ${escapeHtml(report.runId)}</title>
<style>
