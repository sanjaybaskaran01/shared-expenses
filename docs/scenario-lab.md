...
Each run creates four isolated browser contexts at 390×844. Cookies, device keys, IndexedDB, and sync cursors are separate for Ananya, Dev, Mira, and Arjun.
...
2. **Offline replay:** Ananya loses connectivity, records a purchase locally, and reconnects. The server must not see it early, and every device must eventually see it once.
