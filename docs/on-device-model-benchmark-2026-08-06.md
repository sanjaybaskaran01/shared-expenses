...
| Two named payers | Payers `me,ananya`; no invented member | Added `matt`, changed the split to `exact`, then emitted a truncated repeated `not-an-expense` review list. |
| Unknown person | Preserve only known `matt`, route `unknown-person` review | Invented `ananya` as both a payer and participant; no `unknown-person` review. |
...
| Two named payers | Correctly extracted `me=$20` and `ananya=$15`, although it treated the default participant as part of “just us.” | Invalid/truncated output; also invented `matt` as a payer. |
