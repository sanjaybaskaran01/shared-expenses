...
SQLite remains the zero-cost default because it has no separate service, is easy to back up, and suits a single small Tally instance. The entire persistent state lives in a mounted volume, so moving an instance means stopping writes, copying a consistent SQLite backup, and changing the volume/path.
...
|---|---:|---:|---:|---:|---|
| SQLite volume behind Tally API | Lowest | Copy one file/backup | Excellent | Credentials never enter browser | Default now |
| Standard Postgres behind Tally API | Medium | Broad managed/self-hosted choice | Excellent; sync API unchanged | Strong with TLS and least-privilege role | Next adapter |
