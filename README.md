...
- `api.expenses.sanjaybaskaran.com`: Cloudflare Tunnel to `127.0.0.1:3000` on the existing Mac.
...
- Build command: `bun install --frozen-lockfile && VITE_API_URL=https://api.expenses.sanjaybaskaran.com bun --cwd apps/web run build`
...
Install `cloudflared`, create a named tunnel, and adapt `deploy/cloudflared-config.example.yml`. Route only `api.expenses.sanjaybaskaran.com` to `http://127.0.0.1:3000`. Do not add a router port-forward.
